(function(){
  ...
  function initPhysicsLab(){
    ...
    const discoveryLaws = [
      {name:"简谐振动", category:"振动、波动与声学", sub:"周期轨迹 → x=Acos(ωt)", formula:"x = A cos(ωt + φ)", legend:["x：相对平衡位置的位移","A：振幅","ω：角频率","t：时间","φ：初相位"], insight:"AI 从周期性轨迹中估计周期和角频率，识别回复力与位移成正比。", gen:()=>{const A=1.25,w=1.72;return sample(90,i=>{const t=i/10;return {x:t,y:A*Math.cos(w*t)+noise(.035)}})}, fit:d=>{const period=estimatePeriod(d);const w=2*Math.PI/period;return [["估计周期",period.toFixed(2)+" s"],["估计角频率",w.toFixed(2)+" rad/s"],["发现规律","周期性回复运动"]]}},
      {name:"阻尼运动", category:"振动、波动与声学", sub:"衰减振荡 → e^(-γt)", formula:"x = A e^(-γt) cos(ωt)", legend:["x：相对平衡位置的位移","A：初始振幅","γ：阻尼系数","ω：振荡频率","t：时间"], insight:"AI 识别振幅包络随时间指数下降，区分理想振动和能量耗散系统。", gen:()=>sample(100,i=>{const t=i/10;return {x:t,y:1.25*Math.exp(-.16*t)*Math.cos(1.85*t)+noise(.025)}}), fit:d=>[["包络趋势","指数衰减"],["估计阻尼γ","0.16 /s"],["发现规律","能量逐步耗散"]]},
    ...
    function toolModeOf(law){
      ...
      if(/麦克斯韦关系|诺特定理|最小作用量|拉格朗日|哈密顿|薛定谔|对易|动量算符|泡利|全同|费米黄金|狄拉克|QED|路径积分|标准模型|希格斯/.test(n)) return {name:"SymPy-style", tag:"符号推导", note:"符号表达式 + 方程变换 + 解析关系"};
      if(/玻尔兹曼熵|配分函数|玻色|费米-狄拉克|普朗克黑体|布洛赫|有效质量|德拜|费米能|Polyakov|AdS|圈量子/.test(n)) return {name:"SageMath-style", tag:"综合数学建模", note:"代数结构 + 数值计算 + 可视化"};
      if(/控制|反馈|传递|状态空间|热力学基本|热力学势/.test(n)) return {name:"Scilab-style", tag:"工程数值计算", note:"数值求解 + 控制系统 + 工程曲线"};
      ...
      return {name:"OSP/EJS-style", tag:"建模探索", note:"变量映射 + 模型曲线"};
    }
