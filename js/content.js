...
从理论到创新</div>
  ...
  },
  {
    ...
    html:`<section class="chapter physics-lab-chapter" id="s24"><h2>附录 物理规律发现实验室</h2><div class="logic-strip panel"><button type="button" data-goto="s21">承接第10章：真实发现证据</button><b>→</b><strong>本章：多规律自动识别</strong><b>→</b><button type="button" data-goto="s23">导向第21章：企业试点</button></div><div class="physics-lab-grid"><div class="law-menu panel compact-law-nav" id="lawMenu"></div><div class="law-stage panel"><canvas id="lawCanvas" width="880" height="520"></canvas></div><div class="law-result panel"><div class="kicker">AI Discovery Result</div><h3 id="lawTitle">简谐振动</h3><div class="formula" id="lawFormula">x = A cos(ωt + φ)</div><div class="formula-legend dynamic" id="lawLegend"><span>A：振幅</span><span>ω：角频率</span><span>t：时间</span></div><div class="law-controls" id="lawControls"><label>参数强度 <input id="lawYScale" type="range" min="60" max="160" value="100"><b id="lawYScaleValue">1.00x</b></label><label>横轴尺度 <input id="lawXScale" type="range" min="80" max="140" value="100"><b id="lawXScaleValue">1.00x</b></label><label>观测噪声 <input id="lawNoise" type="range" min="0" max="100" value="20"><b id="lawNoiseValue">0.020</b></label></div><p class="lead" id="lawInsight">AI 从周期性轨迹中估计周期和角频率，识别回复力与位移成正比。</p><div class="law-metrics" id="lawMetrics"></div><button class="button" id="lawRegenerate" type="button">重新生成数据</button></div></div></section>`
  },
  {
    title:"终极总结",
    time:"2分钟",
